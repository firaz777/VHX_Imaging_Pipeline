...
int * getInfo(int sleepTime) {
	...
	int TimeElapsed = chrono::duration_cast<chrono::microseconds>(end - start).count();
	static int output[7];
	output[0] = modeI;
	output[1] = stageX;
	output[2] = stageY;
...
void MonitoredSleep(int TimeInMilliseconds) {
	//int timeRunofGetInfo = 70;
	int sleepTime = 50;
	for (int i = 0; i < TimeInMilliseconds; i += 700+sleepTime) {
		int *out = getInfo(sleepTime);
		if (out[0] == -1) {
...
int main(int argc, char* argv[]) {
	...
	MonitoredSleep(movetime);	//Allow stage to finish moving 
	...
	MonitoredSleep((ceil(1000 * waitAutoFocus)));
