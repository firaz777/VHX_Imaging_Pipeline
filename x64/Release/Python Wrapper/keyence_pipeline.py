...
def keyence_pipeline(args):
    ...
    plate_id = args.plate
    if args.retry:
        data_dir = glob.glob(f"Data_{plate_id}_*")
        data_dir.sort()
        dir_tmp = data_dir[-1]    # Retry latest attempt
    else:
        dir_tmp = f"Data_{plate_id}_{int(datetime.datetime.now().timestamp())}"
    working_dir = os.path.abspath(args.dir)
    working_dir_tmp = os.path.join(working_dir, dir_tmp)
    ...
    img_package = assemble_image_sub_package(working_dir_tmp, working_dir, meta_map, dir_tmp[5:])
...
if __name__ == "__main__":
    ...
    parser.add_argument('-r', '--retry', action='store_true', help='Retry last known attempt')
